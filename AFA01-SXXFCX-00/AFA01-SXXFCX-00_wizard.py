...
class FPC_FootprintWizard(FootprintWizardBase.FootprintWizard):
    ...
    def GetName(self):
        return "FPC AFA01-SXXFCX-00"

    def GetDescription(self):
        return "FPC AFA01-SXXFCX-00 Footprint Wizard"

    def GetValue(self):
        pins = self.parameters["Pads"]["n"]
        return ("FPC-SMD_%dP-P1.00_AFA01-S0%dFCC-00" % (pins, pins)) if pins < 10 else ("FPC-SMD_%dP-P1.00_AFA01-S%dFCC-00" % (pins, pins))

    def GenerateParameterList(self):
        self.AddParam( "Pads", "n", self.uInteger, 4 )
        self.AddParam( "Pads", "pitch", self.uMM, 1.0 )
        self.AddParam( "Pads", "width", self.uMM, 0.3 )
        self.AddParam( "Pads", "height", self.uMM, 1.25)

        self.AddParam( "Shield", "shield_to_pad", self.uMM, 2.4 )
        self.AddParam( "Shield", "from_top", self.uMM, 2.525 )
        self.AddParam( "Shield", "width", self.uMM, 1.8 )
        self.AddParam( "Shield", "height", self.uMM, 2.2 )

        self.AddParam( "Courtyard", "spacingX", self.uMM, 3.45 )
        self.AddParam( "Courtyard", "offsetY", self.uMM, 0.359 )
        self.AddParam( "Courtyard", "height", self.uMM, 5.25 )
    ...
    def BuildThisFootprint(self):
        ...
        self.draw.Line( -(offsetX + cty_spacingX), cty_offsetY, -(offsetX + cty_spacingX), cty_offsetY + cty_height )
        self.draw.Line( (offsetX + cty_spacingX), cty_offsetY, (offsetX + cty_spacingX), cty_offsetY + cty_height )
